...
{shopOrder.status=="delivered" && <div className='flex space-x-1 mt-2'>{[1,2,3,4,5].map((star)=>(
  <button className={`text-lg ${selectedRating[item.item._id]>=star?'text-yellow-500':'text-gray-400'}`} onClick={()=>{
    handleRating(item.item._id,star)}}>★</button>
))}</div>}
