    ...
    <div className='w-screen min-h-screen flex flex-col gap-5 items-center bg-[#fff9f6] overflow-y-auto'>
      ...
      <div className='w-full max-w-[800px] flex flex-col gap-5 items-center'>
        ...
        <div className='bg-white rounded-2xl shadow-md p-5 w-[90%] mb-6 border border-orange-100'>
          <h1 className='text-lg font-bold mb-2 text-[#ff4d2d]'>Today Deliveries</h1>
          {todayDeliveries.length === 0 ? (
            <p className='text-sm text-gray-400'>No deliveries completed today</p>
          ) : (
            <>
              <p className='text-sm mb-3 text-gray-600'>Total completed today: <span className='font-semibold text-[#ff4d2d]'>{todayDeliveries.reduce((sum, d) => sum + d.count, 0)}</span></p>
              <div className='space-y-2'>
                {todayDeliveries.map((item, index) => (
                  <div key={index} className='flex justify-between items-center bg-orange-50 px-4 py-2 rounded-lg'>
                    <span className='font-medium text-gray-700'>{item.hour}:00 – {item.hour + 1}:00</span>
                    <span className='font-semibold text-[#ff4d2d]'>{item.count} deliveries</span>
                  </div>
                ))}
              </div>
            </>
        ...
        <div className='bg-white rounded-2xl shadow-md p-5 w-[90%] mb-6 border border-green-200'>
          <h1 className='text-lg font-bold mb-2 text-green-600'>Today's Earnings</h1>
          {totalDeliveriesToday === 0 ? (
            <p className='text-sm text-gray-400'>No earnings yet today</p>
          ) : (
            <div className='flex justify-between items-center'>
              <span className='text-sm text-gray-600'>{totalDeliveriesToday} deliveries × ₹{PER_DELIVERY_AMOUNT}</span>
              <span className='text-xl font-bold text-green-600'>₹{todayEarnings}</span>
            </div>
          )}
        </div>
